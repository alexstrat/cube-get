 ...
 *                                            deduced.
 *         {String}        options.[format] - `json`or `csv`. If no supplied json-parsed data.
...
var buildQuery = function(expression, options) {
  ...
  if(options.start) {
    ...
    if(start === false) throw new Error(options.start + ': invalid rendezvous expression');
  }

  var stop;
  if(options.stop) {
    stop = (options.stop instanceof Date) ? options.stop : rdv(options.stop);
    if(stop === false) throw new Error(options.stop + ': invalid rendezvous expression');
  }
  ...
  if(options.limit) {
    if(typeof options.limit === 'string' && (!start || !stop)) {
      ...
      if(duration === false) throw new Error(options.limit + ': invalid rendezvous duration expression');
